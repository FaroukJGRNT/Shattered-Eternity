...
func _on_enemy_damage_taken(dmg: DamageContainer) -> void:
	...
	for dmg_type in dmg_list.keys():
		...
		if amount > 0:
			...
			dmg_text_instance.position = Vector2(position.x + (player.facing * 5), position.y - 20)
			dmg_text_instance.direction = player.facing
	...
	if dmg.is_crit:
		...
		crit_text_instance.position = Vector2(position.x + (player.facing * 5), position.y - 30)
		crit_text_instance.direction = player.facing
